"""
BERT IMDB Experiment Visualization
====================================
Reads bert_imdb.json + efficiency_detail_bert.json
Outputs 6 PNG figures to the specified output directory.

Usage:
    python plot_bert_imdb.py
"""
...
MAIN_JSON = os.path.join(BASE, r"server_output_bert_imdb\results\bert_imdb.json")
EFF_JSON  = os.path.join(BASE, r"server_output_bert_imdb\results\efficiency_detail_bert\efficiency_detail_bert.json")
OUT_DIR   = os.path.join(BASE, r"output_bert_imdb")
...
def plot_speed():
    ...
    for i, t in enumerate(infer_times):
        ax.text(i, t + 0.008, f"{t:.3f}s", ha="center", fontsize=9,
...
if __name__ == "__main__":
    print(f"\n{'='*50}")
    print(f"  BERT IMDB Visualization")
