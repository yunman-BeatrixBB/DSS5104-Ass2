"""
SetFit IMDB Few-Shot Visualization
====================================
Reads setfit_imdb.json (1%, 5%, 10% only)
Outputs 4 PNG figures to the specified output directory.

Usage:
    python plot_setfit_imdb.py
"""
...
EFF_JSON = os.path.join(BASE, r"server_output_setfit_imdb\results\setfit_imdb.json")
OUT_DIR  = os.path.join(BASE, r"output_setfit_imdb")
...
def plot_seed_stability():
    ...
    for si, seed_val in enumerate(SEEDS):
        ...
        for bar, a in zip(bars, accs):
            ax.text(bar.get_x() + bar.get_width() / 2, a + 0.001,
                    ...
                    color=SEED_COLORS[si], fontweight="bold")
    ...
    ax.set_ylim(min(all_accs) - 0.01, max(all_accs) + 0.01)
...
if __name__ == "__main__":
    print(f"\n{'='*50}")
    print(f"  SetFit IMDB Visualization")
