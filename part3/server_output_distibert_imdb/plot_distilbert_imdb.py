"""
DistilBERT IMDB Experiment Visualization
==========================================
Reads distilbert_imdb.json + efficiency_detail_distilbert.json
Outputs 6 PNG figures to the specified output directory.

Usage:
    python plot_distilbert_imdb.py
"""
...
MAIN_JSON = os.path.join(BASE, r"server_output_distibert_imdb\results\distilbert_imdb.json")
EFF_JSON  = os.path.join(BASE, r"server_output_distibert_imdb\results\efficiency_detail\efficiency_detail_distilbert.json")
OUT_DIR   = os.path.join(BASE, r"output_distilbert_imdb")
...
model_name = main.get("model", "distilbert-base-uncased")
dataset    = main.get("dataset", "imdb")
...
def plot_lr_search():
    ...
    for i, (lbl, acc) in enumerate(zip(labels, accs)):
        ax.text(i, acc + 0.0003, f"{acc:.4f}", ha="center", va="bottom",
                fontsize=10, fontweight="bold",
                color=C["teal"] if i == best_idx else C["muted"])

    ax.set_xlabel("Learning Rate")
    ax.set_ylabel("Validation Accuracy")
    ax.set_title(f"Hyperparameter Search — {model_name} on {dataset}",
                 fontsize=14, fontweight="bold", pad=12)
    ax.set_ylim(min(accs) - 0.005, max(accs) + 0.004)
...
def plot_seed_stability():
    ...
    ax.set_ylim(min(accs) - 0.003, max(accs) + 0.003)
...
def plot_training_curves():
    ...
    fig.suptitle(f"Training Curves — {model_name} on {dataset}",
...
def plot_data_efficiency():
    ...
    for i, (m, s) in enumerate(zip(means, stds)):
        ax.text(i, m + s + 0.003, f"{m:.4f}", ha="center", fontsize=8,
...
if __name__ == "__main__":
    print(f"\n{'='*50}")
    print(f"  DistilBERT IMDB Visualization")
