"""
bert_imdb_v3_merged.py — BERT-base on IMDB (with checkpoint/resume)
DSS5104 Text Classification Assignment (Tier 3)

Merged from:
  - bert_imdb_v3_a100.py  (main pipeline)
  - efficiency_bert_imdb.py (detailed efficiency outputs)
...
MAX_LEN       = 512
BATCH_SIZE    = 48          # A100 40GB can handle 48 (v2 was 16)
NUM_EPOCHS    = 5
PATIENCE      = 2
WARMUP_RATIO  = 0.1
WEIGHT_DECAY  = 0.01
...
def get_device():
    if torch.cuda.is_available():
        ...
        # A100 optimizations: TF32 gives ~2x speedup with negligible precision loss
...
# Merged from efficiency_bert_imdb.py
...
# ═══════════════════════════════════════════════
# EFFICIENCY PLOTS (from efficiency_bert_imdb.py)
# ═══════════════════════════════════════════════
...
def plot_efficiency_confusion(y_true, y_pred, frac, save_dir):
    ...
    ax.set_title(f"BERT IMDB — {frac*100:.0f}% Data — Confusion Matrix")
    plt.tight_layout()
...
def plot_efficiency_classf1(per_class, frac, save_dir):
    ...
    ax.set_title(f"BERT IMDB — {frac*100:.0f}% Data — Per-Class F1")
    ax.set_ylim(0, 1.05)
    plt.tight_layout()
...
def plot_efficiency_summary(all_detail, save_dir):
    ...
    ax.set_title("BERT IMDB — Per-Class F1 Across Data Fractions")
