"""
setfit_agnews.py — SetFit few-shot on AG News (1%, 5%, 10% only)
...
bert_agnews_v3_5080.py for direct comparison.

Usage:
  pip install setfit sentence-transformers==3.3.1 transformers==4.44.2
  python setfit_agnews.py

Optional:
  python setfit_agnews.py --fractions 0.01 0.05 0.10
  python setfit_agnews.py --batch_size 16
"""
...
def load_data():
    ...
    # Combine title + description — same as BERT script
...
def plot_confusion(y_true, y_pred, frac, save_dir):
    ...
    ax.set_title(f"SetFit AG News — {frac*100:.0f}% Data — Confusion Matrix")
    plt.tight_layout()
...
def plot_classf1(per_class, frac, save_dir):
    ...
    colors = ["#e74c3c" if f == min(f1s) else "#2ecc71" if f == max(f1s) else "#3498db"
              for f in f1s]
    ...
    ax.set_title(f"SetFit AG News — {frac*100:.0f}% Data — Per-Class F1")
    ax.set_ylim(0, 1.05)
    plt.tight_layout()
...
def main():
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"  SetFit Few-Shot — AG News", flush=True)
    ...
    # ── Summary ──
    print(f"\n{'='*60}", flush=True)
    print(f"  SetFit AG News — COMPLETE", flush=True)
    print(f"{'='*60}", flush=True)
