"""
distilbert_agnews_v3_5080.py — DistilBERT-base on AG News (5080 optimized, with checkpoint/resume)
DSS5104 Text Classification Assignment (Tier 3)

Optimizations (ported from bert_agnews_v3_5080.py):
  - RTX 5080 (16GB): batch_size=32, AMP (autocast + GradScaler), num_workers=2, persistent_workers
  - TF32 + cudnn.benchmark enabled for Ampere+ GPUs
  ...
  - Part 3 detailed efficiency output: per-class precision/recall/f1, confusion matrix,
    per-class F1 bar charts for EVERY fraction (including 100%)
  - Data efficiency learning-curve plot

Usage:
  python distilbert_agnews_v3_5080.py
"""
...
MAX_LEN       = 128
BATCH_SIZE    = 32          # 5080 16GB — safe for DistilBERT @ 128 tokens
NUM_EPOCHS    = 5
PATIENCE      = 2
WARMUP_RATIO  = 0.1
WEIGHT_DECAY  = 0.01
...
def plot_efficiency_confusion(y_true, y_pred, frac, save_dir):
    ...
    ax.set_title(f"DistilBERT AG News — {frac*100:.0f}% Data — Confusion Matrix")
    plt.tight_layout()
...
def plot_efficiency_classf1(per_class, frac, save_dir):
    ...
    ax.set_title(f"DistilBERT AG News — {frac*100:.0f}% Data — Per-Class F1")
...
def data_efficiency_detail(train_texts, train_labels, val_texts, val_labels,
    ...
    if len(all_detail) > 1:
        ...
        ax.set_title(f"DistilBERT AG News — Per-Class F1 Across Data Fractions")
    ...
    print(f"    - efficiency_detail_distilbert.json  (all numeric results)", flush=True)
